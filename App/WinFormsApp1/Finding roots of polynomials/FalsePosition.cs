...
namespace WinFormsApp1
{
    public partial class FalsePosition : Form
    {
        ...
        {
            org.matheval.Expression expression = new org.matheval.Expression(eq);
            expression.Bind("x", x);
        ...
        public double FalsePos(double xl, double xu, string eq, double eps, int count)
        {
            ...
            {
                xrold = xr;
                xr = Math.Round(xu - ((f(xu, eq) * (xl - xu)) / (f(xl, eq) - f(xu, eq))), 3);
        ...
        private void eqAns_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                label9.Text = FalsePos(xl, xu, eq, error, iNum).ToString();
                label8.Visible = true;
                label9.Visible = true;
